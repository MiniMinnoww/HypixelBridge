...
public class blockEvents implements Listener {
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        if (inGame.i && (event.getPlayer().getName().equalsIgnoreCase(Objects.requireNonNull(Bukkit.getPlayer(playerData_.p.get("player1"))).getName()) || event.getPlayer().getName().equalsIgnoreCase(player2.p.getName()))) {
